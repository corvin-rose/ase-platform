...
namespace Core\App\Extension\JWT;
...
class JWK {
    ...
    public static function parseKey(array $jwk, string $defaultAlg = null): ?Key {
        ...
        switch ($jwk["kty"]) {
            ...
            case "EC":
                ...
                $publicKey = self::createPemFromCrvAndXYCoordinates(
                    $jwk["crv"],
                    $jwk["x"],
                    $jwk["y"]
                );
    ...
    private static function createPemFromCrvAndXYCoordinates(
        ...
        $pem = self::encodeDER(
            ...
            self::encodeDER(
                self::ASN1_SEQUENCE,
                self::encodeDER(self::ASN1_OBJECT_IDENTIFIER, self::encodeOID(self::OID)) .
                    self::encodeDER(
                ...
                self::encodeDER(
                    ...
                    \chr(0x00) . \chr(0x04) . JWT::urlsafeB64Decode($x) . JWT::urlsafeB64Decode($y)
                )
        );
